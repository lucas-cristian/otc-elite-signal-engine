import type { FeatureSnapshot } from '../../common/models/journal-types.js';
import type { Candle, Tick } from '../../common/models/types.js';
...
function normalized(value: number, scale: number | null): number | null {
  if (scale === null || scale === 0) return null;
  return value / scale;
}

function closesOf(candles: readonly Candle[]): number[] {
  return candles.flatMap((candle) => candle.close === null ? [] : [candle.close]);
}

function highsOf(candles: readonly Candle[]): number[] {
  return candles.flatMap((candle) => candle.high === null ? [] : [candle.high]);
}

function lowsOf(candles: readonly Candle[]): number[] {
  return candles.flatMap((candle) => candle.low === null ? [] : [candle.low]);
}

function tickImbalance(ticks: readonly Tick[]): number | null {
  ...
  for (let index = 1; index < ticks.length; index++) {
    ...
    if (current.price > previous.price) up += 1;
    else if (current.price < previous.price) down += 1;
  }
  const total = up + down;
  return total === 0 ? 0 : (up - down) / total;
}

function directionalSequence(candles: readonly Candle[]): number | null {
...
export class FeatureEngine {
  public compute(input: FeatureInput): FeatureSnapshot {
