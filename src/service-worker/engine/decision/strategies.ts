...
function feature(snapshot: FeatureSnapshot, key: string): number | null {
...
export interface Strategy {
  ...
  evaluate(features: FeatureSnapshot, regime: MarketRegimeSnapshot): StrategyEvaluation;
}

export class MomentumStrategy implements Strategy {
  public readonly strategyId = 'MOMENTUM_V1';
  public evaluate(features: FeatureSnapshot, regime: MarketRegimeSnapshot): StrategyEvaluation {
    const momentum = feature(features, 'momentum3');
    const flow = feature(features, 'tickImbalance');
    if (momentum === null || flow === null) return evaluation(this.strategyId, 'NO_TRADE', 0, [], ['WARMUP_MOMENTUM']);
    if (regime.structure === 'CHAOTIC' || regime.structure === 'UNKNOWN') return evaluation(this.strategyId, 'NO_TRADE', 0, [], ['REGIME_BLOCKED']);
    const rawScore = Math.min(1, Math.abs(momentum) * 0.7 + Math.abs(flow) * 0.3);
    if (rawScore < 0.2 || Math.sign(momentum) !== Math.sign(flow)) return evaluation(this.strategyId, 'NO_TRADE', rawScore, [], []);
    const direction: SignalDirection = momentum > 0 ? 'CALL' : 'PUT';
...
export class ReversalStrategy implements Strategy {
  public readonly strategyId = 'REVERSAL_V1';
  public evaluate(features: FeatureSnapshot, regime: MarketRegimeSnapshot): StrategyEvaluation {
    const rejection = feature(features, 'rejection');
    const z = feature(features, 'bollingerZ20');
    const rsi = feature(features, 'rsi14');
    if (rejection === null || z === null || rsi === null) return evaluation(this.strategyId, 'NO_TRADE', 0, [], ['WARMUP_REVERSAL']);
    if (regime.structure !== 'RANGE') return evaluation(this.strategyId, 'NO_TRADE', 0, [], ['REGIME_BLOCKED']);
    const bullish = rejection > 0.2 && z < -1 && rsi < 40;
    const bearish = rejection < -0.2 && z > 1 && rsi > 60;
    if (!bullish && !bearish) return evaluation(this.strategyId, 'NO_TRADE', 0, [], []);
    const direction: SignalDirection = bullish ? 'CALL' : 'PUT';
...
export class ExhaustionStrategy implements Strategy {
  public readonly strategyId = 'EXHAUSTION_V1';
  public evaluate(features: FeatureSnapshot, regime: MarketRegimeSnapshot): StrategyEvaluation {
    ...
    if (acceleration === null || finalSeconds === null || expansion === null) return evaluation(this.strategyId, 'NO_TRADE', 0, [], ['WARMUP_EXHAUSTION']);
    if (regime.structure === 'CHAOTIC') return evaluation(this.strategyId, 'NO_TRADE', 0, [], ['REGIME_BLOCKED']);
...
export class BreakoutStrategy implements Strategy {
  public readonly strategyId = 'BREAKOUT_V1';
  public evaluate(features: FeatureSnapshot, regime: MarketRegimeSnapshot): StrategyEvaluation {
    const compression = feature(features, 'compression');
    const expansion = feature(features, 'expansion');
    const flow = feature(features, 'tickImbalance');
    if (compression === null || expansion === null || flow === null) return evaluation(this.strategyId, 'NO_TRADE', 0, [], ['WARMUP_BREAKOUT']);
    if (regime.structure === 'CHAOTIC') return evaluation(this.strategyId, 'NO_TRADE', 0, [], ['REGIME_BLOCKED']);
    if (compression < 0.15 || expansion < 0.15 || Math.abs(flow) < 0.2) return evaluation(this.strategyId, 'NO_TRADE', 0, [], []);
    const direction: SignalDirection = flow > 0 ? 'CALL' : 'PUT';
...
export class RejectionStrategy implements Strategy {
  public readonly strategyId = 'REJECTION_V1';
  public evaluate(features: FeatureSnapshot, regime: MarketRegimeSnapshot): StrategyEvaluation {
    const rejection = feature(features, 'rejection');
    const imbalance = feature(features, 'tickImbalance');
    if (rejection === null || imbalance === null) return evaluation(this.strategyId, 'NO_TRADE', 0, [], ['WARMUP_REJECTION']);
    if (regime.structure === 'CHAOTIC') return evaluation(this.strategyId, 'NO_TRADE', 0, [], ['REGIME_BLOCKED']);
    if (Math.abs(rejection) < 0.25 || Math.sign(rejection) !== Math.sign(imbalance)) return evaluation(this.strategyId, 'NO_TRADE', 0, [], []);
    const direction: SignalDirection = rejection > 0 ? 'CALL' : 'PUT';
...
export class StrategySelector {
  ...
  public evaluate(features: FeatureSnapshot, regime: MarketRegimeSnapshot): StrategyEvaluation[] {
    if (regime.structure === 'CHAOTIC' || regime.volatility === 'UNKNOWN') return [];
