...
export class QuantPipeline {
  ...
  private isContextualPrimaryTransportHandoff(runtime: AssetRuntime, tick: Tick, gapMs: number): boolean {
    if (runtime.pendingConnectionLossAt !== null) return false;
    if (runtime.currentConnectionId.startsWith('shadow-main-')) return false;
    if (!tick.connectionId.startsWith('shadow-main-')) return false;
    if (runtime.currentPageSessionId !== tick.pageSessionId) return false;
    if (gapMs > this.config.continuityGapAfterMs) return false;
    const previousTick = runtime.latestTick;
    if (previousTick?.sourceTimestampEpochMs === null || previousTick?.sourceTimestampEpochMs === undefined) return false;
    if (tick.sourceTimestampEpochMs === null) return false;
