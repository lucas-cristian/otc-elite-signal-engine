...
export function ema(values, period) {
    ...
    for (let index = period; index < values.length; index++)
        current = (values[index] ?? current) * alpha + current * (1 - alpha);
    return current;
}
export function rsiWilder(values, period) {
...
export function atr(highs, lows, closes, period) {
    if (period <= 0 || highs.length !== lows.length || lows.length !== closes.length || closes.length < period + 1)
        return null;
    const tr = [];
    ...
    if (current === null)
        return null;
    for (let index = period; index < tr.length; index++)
        current = (current * (period - 1) + (tr[index] ?? current)) / period;
    return current;
}
export function stochastic(highs, lows, closes, period) {
    if (period <= 0 || closes.length < period || highs.length !== closes.length || lows.length !== closes.length)
        return null;
...
export function bollingerZ(values, period) {
    ...
    if (avg === null || sd === null || latest === undefined)
        return null;
