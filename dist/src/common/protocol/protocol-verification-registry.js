...
function descriptor(feedHost, eventKind) {
...
const REGISTRY = VERIFIED_FEEDS.flatMap((feedHost) => [
    descriptor(feedHost, 'PRICE_STREAM'),
    descriptor(feedHost, 'PAYOUT'),
]);
export function protocolVerificationRegistry() {
    return REGISTRY;
}
export function resolveProtocolVerification(query) {
    ...
    if (!match)
        return { quality: 'INFERRED', verificationId: null, sourceClockPolicy: null };
    return {
        quality: 'VERIFIED',
