...
if (!manifest.content_security_policy?.extension_pages?.includes('wss://*.po.market')) throw new Error('extension CSP must permit the shadow market WebSocket');
...
if (!contentBundle.includes('SHADOW_RECOVERY_CONTEXT')) throw new Error('content script must forward ephemeral shadow recovery context');
...
if (!backgroundBundle.includes('ShadowMarketConnection')) throw new Error('shadow market connection missing from service worker');
